...
function ToDoList(selected, name, items, userId, toDoListId) {
   ...
   // self.user = ko.observable(user || null);
    self.userId = ko.observable(userId || null);
    ...
    self.removeItem = function (listItem) {
        ...
        $.ajax({
            method: "DELETE",
            url: "/api/listitems/" + listItem.listItemID(),
            contentType: "application/json",
            dataType: "json",
        })
        .done(function (data, textStatus, jqXHR) {
    ...
    self.reloadListItems = function () {
        ...
        $.getJSON("/api/todolist")
                .done(function (todoLists) {
                    ...
                    var mappedData = ko.utils.arrayMap(currentList.listItem, function (item) {
